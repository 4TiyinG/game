...
function solveConstrainedCCD(leg, target, weight, iterations = 3, options = {}) {
    ...
    const minBend = options.minKneeBend ?? THREE.MathUtils.degToRad(2); // 最小膝盖弯曲角，避免腿完全锁死伸直。
    const maxBend = options.maxKneeBend ?? THREE.MathUtils.degToRad(145); // 最大膝盖弯曲角，避免过度折叠。
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        const clampedDistance = THREE.MathUtils.clamp(targetDistance, minReach, maxReach);
...
function projectPoleToChainPlane(restPole, chainDir, target) {
    ...
    if (target.lengthSq() < 0.000001) {
        target.crossVectors(chainDir, new THREE.Vector3(1, 0, 0));
        if (target.lengthSq() < 0.000001) target.crossVectors(chainDir, new THREE.Vector3(0, 0, 1));
    }
