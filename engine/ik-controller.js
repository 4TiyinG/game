class LegIKController {
    ...
    constructor(player, options = {}) {
        ...
        this.soleHalfWidth = options.soleHalfWidth ?? 0.045; // 虚拟脚底左右采样半宽
        this.soleToeExtend = options.soleToeExtend ?? 0.04; // 从 toeBase 继续向鞋尖方向延伸的采样距离
        this.soleHeelExtend = options.soleHeelExtend ?? 0.02; // 从 foot 反向向脚跟延伸的采样距离
        this.soleOffsetY = options.soleOffsetY ?? 0.03; // 脚底采样点垂直下移量（脚踝到实际脚底的距离）
        ...
        this.moveLiftThreshold = options.moveLiftThreshold ?? 0.003; // 移动时脚底上抬触发阈值

        // 胶囊已经上台阶、但脚射线还踩在低处时，把人物 mesh 临时往下拽。
        this.maxMeshStepDrop = options.maxMeshStepDrop ?? 0.25; // mesh 台阶视觉补偿的最大下拽距离
        this.meshBaseY = player.playerModel?.position.y ?? 0; // 人物 mesh 初始本地 y 坐标
        this.meshStepOffsetY = 0; // 当前 mesh 台阶视觉补偿偏移

        // 脚底检测射线（加长到 3.5 以覆盖楼梯/斜坡高度差）
        this.raycaster = new THREE.Raycaster(new THREE.Vector3(), new THREE.Vector3(0, -1, 0), 0, 3.5);
        this.raycaster.firstHitOnly = true;

        // 手机端减少射线采样：仅检测脚跟和脚尖中心 2 个点（非 4 个），降低 GPU 负载
        this._isMobile = /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent) || window.innerWidth < 768;
        ...
        this.meshStepProbeFootSamplePoint = new THREE.Vector3();

        // 碰撞体距离过滤缓存（性能优化：避免对远处网格做射线检测）
        this._filteredColliders = null;
        this._filterX = NaN;
        this._filterZ = NaN;
        this._filterTmpVec = new THREE.Vector3();
    ...
    update(delta = 1 / 60) {
        ...
        // 移动时仍执行 IK（防穿模+台阶补偿），但使用简化采样
        // 移动时不向下吸脚，仅防止穿模和台阶视觉补偿
    ...
    updateFoot(side, delta, moving) {
        ...
        // 站立（非移动）且支撑脚 planted 时，跳过穿透上抬。
        // 原因：soleOffsetY 已把采样点下移到脚底以下，站立足底采样点必然“穿地”，
        // 若再按移动逻辑上抬，会造成平地双脚异常抬起/吸附。
        const liftAmount = (!moving && leg.planted) ? 0 : this.getMovePenetrationLift(leg, footWorld);
        const liftTargetY = footWorld.y + liftAmount;
        let targetY = liftTargetY;

        // 站立（非移动）时，支撑脚向下吸附地面，防止脚底悬浮
        // 移动时只做防穿模上抬，不向下吸脚，避免滑步
        // 直接使用 soleSamples 实际采样点计算悬浮量，避免 footSamplePoint 导致的异常
        // footSamplePoint 基于脚跟/脚尖中心（脚踝下方约0.186），用它计算会产生过大的下拉
        if (!moving && leg.planted) {
            let maxFloat = 0;
            for (const sample of leg.soleSamples) {
                if (!sample.hasHit) continue;
                // 采样点在世界空间已经下移了 soleOffsetY，这里直接比较采样点与命中点的高度差
                const floatY = sample.point.y - sample.hitPoint.y;
                if (floatY > maxFloat) maxFloat = floatY;
            }
            if (maxFloat > 0.005) {
                const desiredFootY = footWorld.y - maxFloat;
                if (desiredFootY < targetY) {
                    targetY = desiredFootY;
                }
            }
        }
        ...
        leg.plantedWeight = THREE.MathUtils.damp(leg.plantedWeight ?? 0, wantedPlantedWeight, 10, delta);
    ...
    // 对一只脚的虚拟脚底点分别向下射线，选择命中高度最高的点作为本帧调试/法线参考。
    castBestFootGround(leg, footWorld, samples = leg.soleSamples, footSamplePoint = leg.footSamplePoint) {
        ...
        // 统一检测全部 4 个采样点（移动端也检测 4 个，确保楼梯/斜坡 IK 精度）
        let bestHit = null;
    ...
    initSoleLocalSamples(leg) {
        ...
        // soleOffsetY 不在此处（local空间）应用，改在 updateSoleSamples 中按世界空间应用
        // 原因：local空间烘焙后，动画脚骨旋转会导致世界坐标严重偏移
    ...
    updateSoleSamples(leg, footWorld, samples = leg.soleSamples, footSamplePoint = leg.footSamplePoint) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // 在世界空间应用脚底下移偏移（不受脚骨旋转影响，始终垂直下移）
            samples[i].point.y -= this.soleOffsetY;
        }
    ...
    applyMeshStepOffset(delta) {
        ...
        const wantedOffset = THREE.MathUtils.clamp(drop, -this.maxMeshStepDrop, 0);
    ...
    setMeshStepOffset(wantedOffset, delta) {
        ...
        this.meshStepOffsetY = THREE.MathUtils.damp(this.meshStepOffsetY, wantedOffset, speed, delta);
        if (Math.abs(this.meshStepOffsetY) < 0.0001) this.meshStepOffsetY = 0;

        // 【修复抖动】不直接覆盖 model.position.y（会与 syncPlayerToModel 冲突）
        // 仅存储偏移量，由主循环在 syncPlayerToModel 之后叠加
        this._pendingMeshOffsetY = this.meshStepOffsetY;
        model.updateMatrixWorld(true);
    }
    ...
    castGroundAtSample(sampleWorld) {
        return this.castGroundFrom(sampleWorld.x, sampleWorld.y + 0.3, sampleWorld.z);
    }
    ...
    castGroundFrom(x, y, z) {
        ...
        const _colliders = this.getFilteredColliders(x, z);
        const hits = this.raycaster.intersectObjects(_colliders, true);
        return hits.find(hit => this.getWorldHitNormal(hit, this.tmpV3).y > 0.18) ?? null;
    }

    // 按距离过滤碰撞体：仅检测角色周围3.5m范围内的网格，大幅减少射线检测开销
    // 缓存机制：角色移动超过0.5m才重新过滤，避免每帧遍历全部碰撞体
    getFilteredColliders(x, z) {
        const dx = x - this._filterX;
        const dz = z - this._filterZ;
        if (this._filteredColliders && dx * dx + dz * dz < 0.25) {
            return this._filteredColliders;
        }

        this._filterX = x;
        this._filterZ = z;

        const colliders = Array.isArray(this.collider) ? this.collider : [this.collider];
        if (colliders.length <= 10) {
            this._filteredColliders = colliders;
            return colliders;
        }

        const maxRadius = 5.0;
        const filtered = [];
        const tmpVec = this._filterTmpVec;

        for (const mesh of colliders) {
            if (!mesh || !mesh.geometry) continue;
            if (!mesh.geometry.boundingSphere) mesh.geometry.computeBoundingSphere();
            const bs = mesh.geometry.boundingSphere;
            if (!bs) { filtered.push(mesh); continue; }

            mesh.updateMatrixWorld();
            tmpVec.copy(bs.center).applyMatrix4(mesh.matrixWorld);
            const mdx = tmpVec.x - x;
            const mdz = tmpVec.z - z;
            const maxDist = maxRadius + bs.radius;
            if (mdx * mdx + mdz * mdz < maxDist * maxDist) {
                filtered.push(mesh);
            }
        }

        this._filteredColliders = filtered.length > 0 ? filtered : colliders;
        return this._filteredColliders;
    }
    ...
    applyPelvis(delta) {
        ...
        const wantedWorldOffset = THREE.MathUtils.clamp(Math.min(left * leftW, right * rightW), -this.maxPelvisDrop, 0.035);

        if (this.pelvisOffset === undefined) this.pelvisOffset = 0;
        this.pelvisOffset = THREE.MathUtils.damp(this.pelvisOffset, wantedWorldOffset, 12, delta);
    ...
    applyLeg(side) {
        ...
        // 穿透修正 — 手机端减少 CCD 迭代次数
        this.solveCCD(leg, leg.smoothedTarget, leg.weight, this._isMobile ? 4 : 8);
        this.preserveFootWorldRotation(leg, animatedFootWorldQ, leg.weight);
        this.alignFootToGround(leg);
        // 平地（法线接近垂直）跳过二次脚底修正，省4次射线/脚
        if (leg.hitNormal.y < 0.95) {
    ...
    preserveFootWorldRotation(leg, animatedWorldQ, weight) {
        ...
        currentWorldQ.slerp(animatedWorldQ, THREE.MathUtils.clamp(weight, 0, 1));
    ...
    correctPostAlignSoleContact(leg) {
        ...
        // 统一检测全部 4 个采样点（移动端也检测 4 个，确保楼梯/斜坡精度）
        ...
        const slopeContactWeight = THREE.MathUtils.clamp(leg.hitNormal.y, 0, 1);
        contactOffset += flatContactBias * slopeContactWeight;
        // contactOffset = THREE.MathUtils.clamp(contactOffset, -0.06, 0.06);
        ...
        this.solveCCD(leg, correctedTarget, leg.weight, this._isMobile ? 2 : 4);
    ...
    alignFootToGround(leg) {
        ...
        const realAngle = 2 * Math.acos(THREE.MathUtils.clamp(alignQ.w, -1, 1));
        if (realAngle > maxTilt) {
            alignQ.slerp(new THREE.Quaternion(), 1 - maxTilt / realAngle);
        }
        alignQ.slerp(new THREE.Quaternion(), 1 - leg.weight * this.footAlignWeight);
